...
class TestMockActivationPolicy:
    ...
    def test_resolve_conflicts_returns_active_conflicts_except_target(
        ...
        active_conflict = mock_factory.create_mock(
            mock_id="active", active=True
        )
        inactive_conflict = mock_factory.create_mock(
            mock_id="inactive", active=False
        )
        same_as_target = mock_factory.create_mock(
            mock_id="target", active=True
        )
